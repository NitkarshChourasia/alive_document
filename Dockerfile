...
# Run as `docker run -it --mount src="$HOME/.cache",target=/root/.cache,type=bind --gpus=all alive_document`, requires Nvidia container toolkit.
...
# If this changes in the future you can `docker build --build-arg device_type=cuda  . -t alive_document` (+GPU argument to be determined).
ARG device_type=cpu
RUN --mount=type=cache,target=/root/.cache python ingest.py --device_type $device_type
COPY . .
ENV device_type=cuda
CMD python run_alive_document.py --device_type $device_type